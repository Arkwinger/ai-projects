...
def mnist_normalize(x01: torch.Tensor) -> torch.Tensor:
    """Normalize a [0,1] tensor to MNIST stats."""
    return (x01 - MNIST_MEAN) / MNIST_STD


# =========================
# PNG / Base64 utilities
# =========================
def png_from_x01(x01: np.ndarray) -> str:
    """Encode a [0,1] grayscale image (28,28) to base64 PNG."""
...
def x01_from_b64_png(b64: str) -> np.ndarray:
    """Decode base64 PNG to [0,1] numpy array (28,28)."""
...
def fetch_challenge(host: str, retries: int = 30, delay: float = 1.0) -> Challenge:
    ...
    for _ in range(max(1, retries)):
        ...
        raise RuntimeError(f"Failed to connect to {host}/challenge: {last_err}")
...
def main() -> None:
    ...
    parser.add_argument(
        "--host",
        default="http://94.237.63.176:41088",
        help="Server base URL",
    )
    parser.add_argument(
        "--weights",
        default="solver/fgsm_weights.pth",
    ...
    for m in safety:
        ...
        cand_pred = int(
            torch.argmax(
                model(mnist_normalize(torch.from_numpy(cand))), dim=1
            ).item()
        )
    ...
    if x_adv is None:
        ...
        adv_pred = int(
            torch.argmax(
                model(mnist_normalize(torch.from_numpy(x_adv))), dim=1
            ).item()
        )
        linf = float(np.max(np.abs(x_adv - chall.x01)))
